...
3. The Agent validates the new file with `growl validate --file <path>` and then
   validates the complete wiki with `growl validate`, fixing any reported errors,
...
run `growl validate --config ./growl.yml`; `--config` is the path to the
...
`source_tracking.enabled` validates formal `sources` records, source existence,
and SHA-256 drift. `path` is relative to the wiki root and `origin` is optional.
...
directory and resolve `wiki_root` from it. `validate` without `--file` validates
the whole wiki, including pages that cannot be reached from `index.md`. It
prints a compact summary by default; use `--details` for individual diagnostics.
